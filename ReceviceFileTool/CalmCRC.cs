...
namespace ReceviceFileTool
{
    public class CalmCRC
    {
        ...
        {
            int crc = 0xFFFF;
            byte[] crc_hl=new byte[2];

            int lut;
            for (int i = 0; i < cmd.Length; i++)
            {
                lut = (crc ^ (int)cmd[i]) & 0xFF;
                crc = (crc >> 8) ^ wCRC16Table[lut];
            }
            crc_hl = System.BitConverter.GetBytes(crc);
            return crc_hl;
        }
        ...
        /// <returns></returns>
        public static byte[] CRC16_C(byte[] data, Boolean IsHighFromt)
        {        
            byte[] bDefault = new byte[] { 0xFF, 0xFF };   //初始化     
            byte[] bMulti = new byte[] { 0xA0, 0x01 };    //多项式值
            byte CRC16Lo;
            byte CRC16Hi;   //CRC寄存器 
            byte CL = bMulti[1];
            byte CH = bMulti[0];   //多项式码&HA001 
